...
for pdf_path, year in test_cases:
    ...
    try:
        ...
        preprocessed_image = preprocessor.preprocess_for_ocr(
            str(pdf_path), page_num=0, save_intermediate=False
        )
        ...
        result = {"year": year, "status": "FAILED", "error": str(e)}
        print(f"  FAILED: {e}")
        print()
...
if successful:
    ...
    print(
        f"  Confidence: {best['confidence']:.1f}%, Missing: {best['missing_pct']:.1f}%, Sectors: {best['sectors']}"
    )
print()
